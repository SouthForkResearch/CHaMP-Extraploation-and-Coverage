...
#####################################################################################
# Cheesy bit to incorporate sinuosity from different files of different types.  
# Hopefully we can cut this out once a good, robust, all inclusive GAA file is available.  Cuz this sucks.

CW_Sin = read.dbf("Clearwater_GAA_PriorityWtrshds_wConfSinu_20160113/Clearwater_GAA_PriorityWtrshds_wConfSinu_20160113.dbf")
CW_Sin = CW_Sin[,c(7,147,148)]

names(CW_Sin)

LWD_Sin_Conf.data = read.csv("GAA_EntTucYFGRUS_LWDSinConf_attributes_20151029.csv",header=T)
LWD_Sin_Conf.data = LWD_Sin_Conf.data[,c(2,4,5)]
names(LWD_Sin_Conf.data)
names(CW_Sin) = names(LWD_Sin_Conf.data)

Sin.data = rbind(CW_Sin, LWD_Sin_Conf.data)

LemhiSin = read.csv("Lemhi_Sin.csv", header=T)
Sin.data = rbind(Sin.data, LemhiSin)
SFS_Sin = read.csv("SFS_Sin.csv", header=T)
Sin.data = rbind(Sin.data, SFS_Sin)
BigCreek_Sin = read.csv("BigCreek_Sin.csv", header=T)
names(BigCreek_Sin)
names(Sin.data)
Sin.data = rbind(Sin.data, BigCreek_Sin)
...
names(GAA.data)
GAA.data$Site_ID
idx = match(GAA.data$Site_ID,Sin.data$Site_ID)
idx
GAA.data$Sin = Sin.data$Sin[idx]
GAA.data$Sin[GAA.data$Sin <1] = NA
GAA.data$Conf = Sin.data$Conf[idx]
GAA.data$Conf[GAA.data$Conf < 0] = NA
#############################################################################
#############################################################################


#temp = GAA.data[is.na(GAA.data$Sin)==FALSE,]
...
#PredictedWatersheds = c("South Fork Clearwater", "Lower Clearwater","Lolo","Lochsa")
